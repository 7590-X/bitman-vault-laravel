{{--
    Componente independiente de la lista vertical de Llaves SSH.
--}}
<div class="flex flex-col h-full bg-slate-900 border-r border-slate-700/80 text-slate-100 min-w-l">
    ...
    <div class="p-4 border-b border-slate-700/80 flex items-center justify-between shrink-0 bg-slate-900/90 backdrop-blur">
        <div class="flex items-center gap-2.5">
            <x-icon.key class="w-5 h-5 text-blue-400" />
            <h3 class="text-base font-bold tracking-tight text-white">SSH Keys</h3>
        </div>
        <span class="text-xs px-2 py-1 rounded-full bg-slate-800 border border-slate-700/60 text-slate-300 font-mono" x-text="llaves.length">0</span>
    </div>
    ...
    <div class="flex-1 overflow-y-auto min-h-0 divide-y divide-slate-800/60">
        ...
        <template x-if="cargando">
            <div class="p-6 text-center text-slate-400 space-y-3">
                ...
                <p class="text-xs font-medium">Cargando llaves SSH...</p>
            </div>
        </template>

        {{-- Estado Vacío: Sin Llaves Registradas --}}
        <template x-if="!cargando && llaves.length === 0">
            <div class="p-8 text-center flex flex-col items-center justify-center h-full min-h-[220px]">
                <div class="w-12 h-12 rounded-full bg-slate-800/80 border border-slate-700 flex items-center justify-center text-slate-400 mb-3 shadow-inner">
                    <x-icon.key class="w-6 h-6 opacity-60" />
                </div>
                <h4 class="text-sm font-semibold text-slate-200">Sin Llaves SSH Registradas</h4>
                <p class="text-xs text-slate-400 mt-1 max-w-[200px]">No has guardado ninguna llave SSH aún en tu bóveda.</p>
            </div>
        </template>

        {{-- Renderizado dinámico de la lista de Llaves SSH --}}
        <template x-if="!cargando && llaves.length > 0">
            <div class="divide-y divide-slate-800/50">
                <template x-for="llave in llaves" :key="llave.id">
                    <div
                        ...
                        :class="[
                            llaveSeleccionada && llaveSeleccionada.id === llave.id && !creando ? 'bg-slate-800 border-l-4 border-l-blue-500 font-semibold' : '',
                        ...
                        <div class="min-w-0 flex-1">
                            <h4 class="text-sm font-semibold text-slate-100 truncate group-hover:text-white transition-colors" x-text="llave.nombre"></h4>
                            <p class="text-xs text-slate-400 truncate mt-0.5 font-sans">Llave SSH</p>
                        </div>
                    </div>
                </template>
            </div>
    ...
    {{-- Pie inferior estático con botón siempre visible para Crear Llave --}}
