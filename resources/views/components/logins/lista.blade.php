    ...
    <div class="flex-1 overflow-y-auto min-h-0 divide-y divide-slate-800/60">
        ...
        <template x-if="cargando">
            <div class="p-6 text-center text-slate-400 space-y-3">
                ...
                <p class="text-xs font-medium">Cargando credenciales...</p>
            </div>
        </template>

        {{-- Estado Vacío: Sin Logins Registrados --}}
        <template x-if="!cargando && logins.length === 0">
            <div class="p-8 text-center flex flex-col items-center justify-center h-full min-h-[220px]">
                <div class="w-12 h-12 rounded-full bg-slate-800/80 border border-slate-700 flex items-center justify-center text-slate-400 mb-3 shadow-inner">
                    <x-icon.globe class="w-6 h-6 opacity-60" />
                </div>
                <h4 class="text-sm font-semibold text-slate-200">Sin Logins Registrados</h4>
                <p class="text-xs text-slate-400 mt-1 max-w-[200px]">No has guardado ninguna credencial aún en tu bóveda.</p>
            </div>
        </template>

        {{-- Renderizado dinámico de la lista de Logins --}}
        <template x-if="!cargando && logins.length > 0">
