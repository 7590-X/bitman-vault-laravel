...
@section('contenido')
<div x-data="{ currentRoute: window.location.hash || '#/logins' }" @hashchange.window="currentRoute = window.location.hash" class="h-full w-full">

    {{-- VISTA: Logins (Credenciales) --}}
    <div x-show="currentRoute === '#/logins'" class="h-full w-full">
        <x-logins.index />
    </div>
